...
class ObjectCounterVideo():
	...
	def count_objects_in_video(self):
		frame_index = 0
		W, H = (None, None)
		total_num_frames = self.determine_number_of_frames_in_video()
		...
		while True:
			...
			blob_img = cv2.dnn.blobFromImage(frame, 1 / 255.0, (416, 416), swapRB=True, crop=False)
			...
			boxes = []
			confidences = []
			class_ids = [] # TODO change class ID
			...
			nms_boxes = cv2.dnn.NMSBoxes(boxes, confidences, self.confidence_threshold, self.nms_threshold)
			...
			if len(tracked_boxes) > 0:
				...
				for box in tracked_boxes:
					...
					if index_ids[i] in previous_box_detections:
						...
						if self._is_object_box_intersecting_with_line(p0, p1, self.line_coords[0], self.line_coords[1]):
			...
			# draw line
			cv2.line(frame, self.line_coords[0], self.line_coords[1], self.line_color, self.line_width)

			# draw counter
			for idx, (key, value) in enumerate(self.instances_count.items()):
				cv2.putText(frame, f"# of {key}: {self.instances_count[key]}", (30,50 + idx*40), cv2.FONT_HERSHEY_DUPLEX, 1.0, (0, 0, 255), 2)

			# check if the video writer is None
			if self.video_writer is None:
				# initialize our video writer
				fourcc = cv2.VideoWriter_fourcc(*"MJPG")
				self.video_writer = cv2.VideoWriter(self.output_video_path, fourcc, 30, (frame.shape[1], frame.shape[0]), True)
